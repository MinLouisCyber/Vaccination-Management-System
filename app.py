...
if db_url:
    ...
    app.config.update(
        ...
        SQLALCHEMY_DATABASE_URI=db_url, # Dùng chuỗi đã chuẩn hóa
...
def generate_certificate_pdf(profile, appointment):
    # ================= ĐƯA KHAI BÁO FONT VÀO TRONG HÀM ================= #
    from reportlab.pdfbase import pdfmetrics
    from reportlab.pdfbase.ttfonts import TTFont
    
    try:
        # Cố gắng load file font arial.ttf (phải có file này trong thư mục chứa app.py)
        ...
        try:
            ...
            MAIN_FONT = 'Arial'
            BOLD_FONT = 'Arial' # Nếu không có arialbd.ttf thì dùng tạm Arial thường
    except Exception as e:
        print(f"CẢNH BÁO FONT: {e}")
        # Nếu không tìm thấy file arial.ttf, quay về dùng font mặc định (sẽ bị lỗi ô vuông)
        MAIN_FONT = 'Helvetica'
    ...
    title_style = ParagraphStyle(
        ...
        spaceAfter=20,
        fontName=BOLD_FONT # Đã dùng biến BOLD_FONT
    )
    ...
    normal_style = ParagraphStyle(
        ...
        fontSize=12,
        leading=16,
        fontName=MAIN_FONT # Đã dùng biến MAIN_FONT
    )
    
    # ---------------- Watermark Background ---------------- #
    p.saveState()
    p.setFillColor(colors.HexColor('#f0f0f0'))
    p.setFont(BOLD_FONT, 60) # Đã dùng biến BOLD_FONT
    ...
    # Header text (Translated to Vietnamese & Font applied)
    header_text_left = Paragraph(
        f"<font name='{BOLD_FONT}'>Bộ Y tế Việt Nam</font><br/><font name='{MAIN_FONT}'>Hệ thống tiêm chủng quốc gia</font>", 
        ParagraphStyle('LeftHeader', parent=normal_style, fontSize=12, textColor=colors.white, alignment=0)
    )
    header_text_right = Paragraph(
        f"<font name='{BOLD_FONT}'>Hồ sơ tiêm chủng</font><br/><font name='{BOLD_FONT}'>Chứng nhận điện tử</font>", 
    ...
    section_title = Paragraph(f"<font name='{BOLD_FONT}'>Thông tin cá nhân</font>", ParagraphStyle('SectionTitle', parent=normal_style, fontSize=14, textColor=colors.HexColor("#003366")))
    ...
    info_table.setStyle(TableStyle([
        ('FONTNAME', (0,0), (-1,-1), MAIN_FONT), # Đã dùng biến MAIN_FONT
        ('FONTSIZE', (0,0), (-1,-1), 12),
...
def dashboard():
    if current_user.role == ROLE_ADMIN:
        ...
        today = date.today()
        # Vaccination history: only appointments with status 'Completed'
        vaccination_history = [appt for appt in appointments if appt.status == 'Completed']
        # Upcoming reminders: status is 'Pending' or 'Confirmed' and date >= today
        upcoming_reminders = [appt for appt in appointments if appt.status in ('Pending', 'Confirmed') and appt.appointment_date >= today]
        return render_template(
            'user_dashboard.html',
            profiles=profiles,
...
def book_appointment():
    if request.method == 'POST':
        ...
        # Continue with the rest of your validation and processing
...
def cancel_appointment(appointment_id):
    ...
    # Restore inventory: find the inventory batch for this vaccine and increment quantity
    inventory_item = (
        db.session.query(Inventory)
        .join(stored_at, Inventory.inventory_id == stored_at.c.inventory_id)
...
def admin_complete_appointment(id):
    """Admin route to mark an appointment as Completed (vaccination done)."""
...
@app.route('/admin/appointment/<int:id>/confirm', methods=['POST'])
@login_required
@role_required([ROLE_ADMIN])
def admin_confirm_appointment(id):
    """Admin route to confirm an appointment (Pending → Confirmed)."""
    appointment = Appointment.query.get_or_404(id)
...
def view_certificates():
    ...
    profile_appointments = {}
    for profile in profiles:
        # Only show appointments with status 'Completed'
        appointments = (Appointment.query
...
def download_certificate(appointment_id):
    ...
    # Security check: only allow download for completed appointments
    if appointment.status != 'Completed':
